...
export default function PresupuestoClient({
    ...
    try {
      ...
      if (result?.presupuesto) {
        ...
        if (presupuestoData.length > 0) {
          const mensual = presupuestoData[0]?.presupuesto_mensual || 0
          const total = presupuestoData[0]?.total_presupuesto || (mensual * 12)
          setPresupuestoMensual(mensual)
          setPresupuestoTotal(total)
          setCurrentYearPresupuestoTotal(total)
        } else {
  ...
  const { availableMeses, availableAños } = useMemo(() => {
    ...
    if (!selectedAño || selectedAño === currentYear) mesesSet.add(mesActual)
    if (mesesSet.size === 0) mesesSet.add(selectedAño === currentYear ? mesActual : "Enero")
  ...
  // Órdenes filtradas por departamento, mes y año (sin inversiones)
  ...
  // Gasto total del año seleccionado
  ...
  const presupuestoMensualRecomendado = useMemo(() => {
    ...
    return presupuestoActual / mesesRestantes
        ...
        <div className="col-span-1">
          <div className="grid gap-6">
            ...
            <div className="bg-white rounded-lg p-6 shadow-sm">
              <div className="flex justify-between items-start">
                <div className="w-1/2 pr-4">
                  ...
                  <div className="text-4xl font-bold text-gray-400">{formatCurrency(presupuestoTotal)}</div>
                </div>
            ...
            <div className="bg-white border border-gray-200 rounded-lg p-6">
                ...
                <div className="relative group">
                  ...
                  <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 hidden group-hover:block bg-white border border-gray-200 rounded p-3 shadow-lg z-50 w-80">
                    <div className="text-xs">
                      ...
                      <p className="mb-2">Se divide el presupuesto restante entre los meses que quedan del año actual.</p>
              ...
              <div className="text-xs text-gray-500 mt-1">Recomendación: {formatCurrency(presupuestoMensualRecomendado)}/mes</div>
            </div>
                  ...
                  {filteredOrdenes.length > 0 ? (
                    filteredOrdenes.map((item, index) => (
                      <tr key={`${item.idOrden}-${index}`} className="border-t border-gray-200">
                        ...
                        <td className="py-2 px-3 text-right w-1/6">{formatCurrency(parseFloat(item.Importe))}</td>
                      </tr>
                    ))
                  ) : (
                    <tr>
